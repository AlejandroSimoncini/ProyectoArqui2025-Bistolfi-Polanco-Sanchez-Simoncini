...
var jwtKey = []byte("clave") // Clave para verificar los tokens (no se comparte)

// middleware para autenticar usuarios, verifica si el token JWT es válido
func AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		tokenString := c.GetHeader("Authorization") // Obtener el token del encabezado
		...
		tokenStr := strings.TrimPrefix(tokenString, "Token ") //quitar el prefijo "Token "
		...
		//si el token es válido, extraer los datos (claims)
